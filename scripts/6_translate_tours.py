...
USAGE:
  python 6_translate_tours.py --all                    # Translate all companies, all languages
  python 6_translate_tours.py --company cruisewhitsundays  # Translate one company
  python 6_translate_tours.py --languages zh ja ko     # Only translate to specific languages
...
TRANSLATE_FIELDS = [
    ...
    'age_requirements',
    'ideal_for',
    # Additional fields to translate
    'duration',
    'departure_location',
    'departure_times',
    'price_tiers',
...
class TourTranslator:
    def __init__(self):
        ...
        if self.deepl_key and deepl:
            try:
                self.deepl_translator = deepl.Translator(self.deepl_key)
                print("[OK] DeepL API initialized")
    ...
    def translate_text(self, text, target_lang_code, service='deepl'):
        ...
        try:
            if service == 'deepl' and self.deepl_translator:
                ...
                print("  [!] No translator available, keeping original text")
                return text
        except Exception as e:
...
def organize_csvs():
    ...
    # Find all *_cleaned_with_media.csv files in project root
    csv_pattern = os.path.join(project_root, 'tours_*_cleaned_with_media.csv')
    ...
    for csv_file in csv_files:
        ...
        company = filename.replace('tours_', '').replace('_cleaned_with_media.csv', '')
        companies.append(company)
        
        # Create directory structure
        en_dir = os.path.join(project_root, 'data', company, 'en')
        os.makedirs(en_dir, exist_ok=True)
        
        # Move/copy file
        dest_path = os.path.join(en_dir, filename)
        if not os.path.exists(dest_path):
            import shutil
            shutil.copy2(csv_file, dest_path)
            print(f"  [OK] Organized: {filename} -> {dest_path}")
        else:
            print(f"  [SKIP] Already organized: {dest_path}")
    
    return companies


def translate_company(company, languages=None, force=False):
    """Translate a single company's CSV to all specified languages"""
    project_root = get_project_root()
    
    lang_codes = languages if languages else list(LANGUAGES.keys())
    ...
    source_csv = os.path.join(project_root, 'data', company, 'en', f'tours_{company}_cleaned_with_media.csv')
    ...
    for lang_code in lang_codes:
        output_dir = os.path.join(project_root, 'data', company, lang_code)
        output_csv = os.path.join(output_dir, f'tours_{company}_cleaned_with_media.csv')
        
        if os.path.exists(output_csv) and not force:
            print(f"  [SKIP] {LANGUAGES[lang_code]['name']} already exists, skipping...")
            continue
        
        try:
            translator.translate_csv(source_csv, output_csv, lang_code)
        except Exception as e:
            print(f"  [ERROR] Error translating to {lang_code}: {e}")
            continue
    
    print(f"\n[DONE] {company} translation complete!")
    return True
...
def translate_all_companies(languages=None, force=False):
    ...
    if not companies:
        print("[ERROR] No companies found to translate")
        return
    ...
    for company_idx, company in enumerate(companies, 1):
        ...
        for lang_code in lang_codes:
            output_csv = os.path.join(project_root, 'data', company, lang_code, f'tours_{company}_cleaned_with_media.csv')
...
def main():
    ...
    if args.all:
        ...
        translate_company(args.company, args.languages, args.force)
    else:
        parser.print_help()
